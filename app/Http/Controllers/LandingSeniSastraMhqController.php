...
class LandingSeniSastraMhqController extends Controller
{
	...
	public function store(Request $request)
	{
		 ...
		 */
		$row_id = DB::table('pendaftaran_senisastra_mhq')
			->insertGetId([
		...
		$nama_file = [
			'ktm' => [
				'peserta' => $ktm_peserta ? $ktm_peserta->store('senibudaya/senisastra/mhq/' . $row_id, ["disk" => 'pendaftaran']) : "",
			],
			'foto' => [
				'peserta' => $foto_peserta ? $foto_peserta->store('senibudaya/senisastra/mhq/' . $row_id, ["disk" => 'pendaftaran']) : "",
			],
		];
	...
	public function success()
	{
		if (session('status')) {
			...
			return redirect()->route('landing.senibudaya.senisastra.mhq.index');
		}
	}
}
