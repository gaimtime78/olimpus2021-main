...
class DashboardPosterController extends Controller
{
    public function index(Request $request) {
		$table_data_not_confirmed = DB::table('pendaftaran_poster')
			->where('konfirmasi', '=', false)
			->orderBy('id', 'ASC')
			->get();
		$table_data_confirmed = DB::table('pendaftaran_poster')
			->where('konfirmasi', '=', true)
			->orderBy('id', 'ASC')
			->get();
		return view('dashboard.pendaftaran.keilmiahan.poster', [
	...
	public function update(Request $request) {
		...
		return redirect()->route('pendaftaran.poster.index');
	}
}
