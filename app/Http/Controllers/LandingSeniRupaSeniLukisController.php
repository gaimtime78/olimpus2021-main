...
class LandingSeniRupaSeniLukisController extends Controller
{
	public $cabang_lomba = 'Seni Lukis';
	...
	public function store(Request $request) {
		 ...
		 */
		$row_id = DB::table('pendaftaran_senirupa_senilukis')
			->insertGetId([
		...
		$nama_file = [
			'ktm' => [
				'peserta' => $ktm_peserta ? $ktm_peserta->store('senibudaya/senirupa/senilukis/' . $row_id, ["disk" => 'pendaftaran']) : "",
			],
			'foto' => [
				'peserta' => $foto_peserta ? $foto_peserta->store('senibudaya/senirupa/senilukis/' . $row_id, ["disk" => 'pendaftaran']) : "",
			],
		];
		 ...
		 */
		DB::table('pendaftaran_senirupa_senilukis')
			->where('id', '=', $row_id)
			->update([
		...
		return redirect()->route('landing.senibudaya.senirupa.senilukis.success')->with('status', 'SUKSES!');
	}
	...
	public function success() {
		if (session('status')) {
			...
			return redirect()->route('landing.senibudaya.senirupa.senilukis.index');
		}
	}
}
