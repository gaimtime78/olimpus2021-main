...
class LandingSeniSastraCiptaPuisiController extends Controller
{
	public $cabang_lomba = 'Cipta Puisi';
	public $whatsapp = 'https://api.whatsapp.com/send?phone=6285113355202';
	public $pamflet = 'img/senibudaya/senisastra/ciptapuisi/pamflet_revisi.png';
	...
	public function store(Request $request) {
		 ...
		 */
		$row_id = DB::table('pendaftaran_senisastra_ciptapuisi')
			->insertGetId([
		...
		$nama_file = [
			'ktm' => [
				'peserta' => $ktm_peserta ? $ktm_peserta->store('senibudaya/senisastra/ciptapuisi/' . $row_id, ["disk" => 'pendaftaran']) : "",
			],
			'foto' => [
				'peserta' => $foto_peserta ? $foto_peserta->store('senibudaya/senisastra/ciptapuisi/' . $row_id, ["disk" => 'pendaftaran']) : "",
			],
		];
		...
		return redirect()->route('landing.senibudaya.senisastra.ciptapuisi.success')->with('status', 'SUKSES!');
	}
	...
	public function success() {
		if (session('status')) {
			...
			return redirect()->route('landing.senibudaya.senisastra.ciptapuisi.index');
		}
	}
}
