...
class LandingSeniRupaKomikStripController extends Controller
{
	public $cabang_lomba = 'Komik Strip';
	...
	public function index()
	{
		...
		return view('landing.senibudaya.senirupa.komikstrip', [
			'api' => $api,
	...
	public function store(Request $request) {
		 ...
		 */
		$row_id = DB::table('pendaftaran_senirupa_komikstrip')
			->insertGetId([
		...
		$nama_file = [
			'ktm' => [
				'peserta' => $ktm_peserta ? $ktm_peserta->store('senibudaya/senirupa/komikstrip/' . $row_id, ["disk" => 'pendaftaran']) : "",
			],
			'foto' => [
				'peserta' => $foto_peserta ? $foto_peserta->store('senibudaya/senirupa/komikstrip/' . $row_id, ["disk" => 'pendaftaran']) : "",
			],
		];
		...
		return redirect()->route('landing.senibudaya.senirupa.komikstrip.success')->with('status', 'SUKSES!');
	}
	...
	public function success() {
		if (session('status')) {
			...
			return redirect()->route('landing.senibudaya.senirupa.komikstrip.index');
		}
	}
}
