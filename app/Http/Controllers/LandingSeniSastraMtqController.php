...
class LandingSeniSastraMtqController extends Controller
{
	public $cabang_lomba = 'Musabaqah Tilawatil Quran';
	...
	public function index() {
		...
		return view('landing.senibudaya.senisastra.mtq', [
			'api' => $api,
	...
	public function store(Request $request) {
		 ...
		 */
		$row_id = DB::table('pendaftaran_senisastra_mtq')
			->insertGetId([
		...
		$nama_file = [
			'ktm' => [
				'peserta' => $ktm_peserta ? $ktm_peserta->store('senibudaya/senisastra/mtq/' . $row_id, ["disk" => 'pendaftaran']) : "",
			],
			'foto' => [
				'peserta' => $foto_peserta ? $foto_peserta->store('senibudaya/senisastra/mtq/' . $row_id, ["disk" => 'pendaftaran']) : "",
			],
		];
		 ...
		 */
		DB::table('pendaftaran_senisastra_mtq')
			->where('id', '=', $row_id)
			->update([
		...
		return redirect()->route('landing.senibudaya.senisastra.mtq.success')->with('status', 'SUKSES!');
	}
	...
	public function success() {
		if (session('status')) {
			...
			return redirect()->route('landing.senibudaya.senisastra.mtq.index');
		}
	}
}
