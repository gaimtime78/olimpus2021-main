...
class LandingSpeechController extends Controller
{
	...
	public $pamflet = 'img/keilmiahan/speech/pamflet2.jpeg';
	...
	public function store(Request $request)
	{
		$request->validate([
			...
			"jurusan_angkatan_peserta" => "required",
		 ...
		 */
		$row_id = DB::table('pendaftaran_speech')
			->insertGetId([
				'email' => $request->input('email'),
				'fakultas' => $request->input('fakultas'),
				'nama_peserta' => $request->input('nama_peserta'),
				'jurusan_angkatan_peserta' => $request->input('jurusan_angkatan_peserta'),
		...
		$nama_file = [
			'ktm' => [
				'peserta' => $ktm_peserta ? $ktm_peserta->store('keilmiahan/speech/' . $row_id, ["disk" => 'pendaftaran']) : "",
			],
			'foto' => [
				'peserta' => $foto_peserta ? $foto_peserta->store('keilmiahan/speech/' . $row_id, ["disk" => 'pendaftaran']) : "",
			],
		];
