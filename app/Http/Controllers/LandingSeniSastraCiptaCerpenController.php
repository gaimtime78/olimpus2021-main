...
class LandingSeniSastraCiptaCerpenController extends Controller
{
	...
	public function index()
	{
		...
		return view('landing.senibudaya.senisastra.ciptacerpen', [
			'api' => $api,
	...
	public function store(Request $request) {
		 ...
		 */
		$row_id = DB::table('pendaftaran_senisastra_ciptacerpen')
			->insertGetId([
		...
		$nama_file = [
			'ktm' => [
				'peserta' => $ktm_peserta ? $ktm_peserta->store('senibudaya/senisastra/ciptacerpen/' . $row_id, ["disk" => 'pendaftaran']) : "",
			],
			'foto' => [
				'peserta' => $foto_peserta ? $foto_peserta->store('senibudaya/senisastra/ciptacerpen/' . $row_id, ["disk" => 'pendaftaran']) : "",
			],
		];
		...
		return redirect()->route('landing.senibudaya.senisastra.ciptacerpen.success')->with('status', 'SUKSES!');
	}
	...
	public function success() {
		if (session('status')) {
			...
			return redirect()->route('landing.senibudaya.senisastra.ciptacerpen.index');
		}
	}
}
